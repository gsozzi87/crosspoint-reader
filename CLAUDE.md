...
- **Mascota** (`examples/Apps/mascota.lua`, 24 KB; créditos y reglas en `docs/ws397/MASCOTA.md`): tamagotchi
  con **dibujos ajenos, como pidió el dueño** ("no las generes tú"), y **es una perrita salchicha** ("pero
  quiero una perrita salchicha"): la mascota `dachshund` de openclaw-tamagotchi de Artem Katolikov
  (GitHub, **MIT**; idle/walk/sleep de 32 × 32, recortados a 32 × 16 y pintados a escala 7; el cuadro de
  "se fue" es el de quieta dado vuelta). Convertida por paleta y no por umbral (el ojo desaparecía por
  luminancia). Los iconos y el huevo son de OpenCritter de SuperMechaCow (**MIT**). La primera versión era una
  gatita CC0 de Segel (OpenGameArt) y duró un commit. Cuatro barras (hambre, ánimo, energía, higiene) que
  bajan con el reloj de verdad (`cp.time().epoch`, tope de 12 h por ausencia, guardado en `cp.save`);
  Alimentar, Jugar (mayor o menor, 3 rondas), Dormir, Limpiar, Info; nombre por voz al nacer; sacudir la
  despierta, boca abajo la duerme; 36 h de descuido y "se fue" (huevo nuevo). Repinta sólo cuando cambia algo
  (cuadro cada 4 s). Descartados por licencia o legibilidad: el Dachshund CC0 de zwonky (16 × 16, a 1 bit son
  dos manchas), picotamachibi (sin licencia), Matagotchi (GPL), ToffeeCraft (sin redistribución).
