...
| 1.10 | Ajustes desde `/board`: lugar del clima (buscador de ciudades), idioma de la interfaz, voz hablada, idioma del traductor y volumen de la música. Van en `settings{rev,...}` de `GET /api/hub` y el aparato los aplica al sincronizar solo si `rev` subió. La página muestra además el clima que ve el servidor (o por qué está vacío) y hace cuánto que el aparato no sincroniza | ✅ | 1.5.32. Falta orden de mosaicos y widgets, hora de sincronización y unidad °C/°F |
...
| 2.8 | Mosaico Tiempo: temporizador (1 a 60 min por menú, cualquier duración por voz), cronómetro y Pomodoro (25/5 con rondas), dígitos grandes de 7 segmentos, OK pausa/sigue, **Atrás sale dejándolo corriendo** y Atrás largo cancela, pitido o voz al terminar. Por voz: "poné 20 segundos" o "alarma a las 7" abre el temporizador con el tiempo ya puesto | ✅ | 1.5.33: la unidad ya no se pierde ("20 segundos" era 20 minutos: el prompt solo enseñaba minutos, más una red de contención en el servidor) y la respuesta hablada se termina antes de abrir el reloj. 1.5.32: hasta 1.5.31 Atrás (la única salida) borraba el temporizador y pausar también, el cronómetro no se guardaba en ningún lado, `preventAutoSleep()` impedía justo el sueño que lo hace sonar, y el re-sleep por wake espurio del botón dormía sin armar el despertador. Ahora todo el estado vive en `HubStore` como tiempo absoluto, el hub lo muestra (barra + punto en el mosaico) y suena desde cualquier pantalla tranquila |
...
| # | Función | Estado | Notas |
|---|---------|--------|-------|
| 3.1 | Biblia (mosaico Biblia): libros → capítulos → texto paginado; cada capítulo se trae del servidor (Reina-Valera, King James, Bible de l'Épée, Schlachter, Almeida, Sinodal según el idioma) y queda en la SD (`/.crosspoint/bible/<lang>/`), marcado con un punto en la lista; recuerda el último lugar leído. Atrás largo = por voz: "Juan 3 16" abre directo, "salmo 23" también, "misericordia y verdad" busca en todo el texto y lista los versículos (sin LLM: parser de referencias y búsqueda en el servidor) | ✅ | 1.5.36: la última fila de la lista de libros baja la **Biblia entera** a la SD (3,8 MB en español, un archivo por libro) y con eso leer, resolver la cita y buscar texto pasan a ser offline — lo único que sigue necesitando el servidor es pasar la voz a texto. La lógica de recorte de capítulos, de citas y de búsqueda está probada de escritorio con g++. 1.5.93: leyendo un capítulo, **OK abre el menú** igual que en el lector (buscar una palabra en el diccionario instalado, preguntar sobre el capítulo, buscar por voz, elegir capítulo), y el cursor de palabras usa el mismo `dictlookup` que el lector |
| 3.2 | Versículo del día y frase del día en el hub: `GET /api/hub` trae `verse{ref,text}` de la Biblia del idioma; el widget alterna versículo (días pares) y frase cuando no hay eventos | ✅ | 1.5.27 |
| 3.3 | Reproductor MP3 desde la SD con pinta de Winamp (`MusicActivity`): carpetas de `/Music` como playlists; ventana principal con contador de 7 segmentos, marquesina "artista - título" (ID3v2/v1 o nombre de archivo), kbps y kHz, barra de posición, botones de transporte, SHUF/REP y volumen; playlist numerada con el tema actual marcado. UP/DOWN eligen, OK toca o pausa, Atrás vuelve a carpetas (sigue sonando), Atrás largo: detener, siguiente, anterior, aleatorio, repetir, volumen. Decodificador Helix (`lib/HelixMp3`) alimentando al `AudioManager` del SDK como fuente WAV; pausa = silencio (el SDK no pausa) | ✅ | 1.5.36: rediseñado al tamaño de la pantalla (título y artista grandes, contador de 68 px, barra gruesa, botones de 56x38, filas de 38 px). Antes copiaba las proporciones de la skin original de 275x116 y en el aparato quedaba minúsculo. Falta: seguir sonando al salir al hub, saltar dentro del tema |
| 3.4 | Noticias (mosaico Noticias, `NewsActivity`): feeds RSS/Atom cargados en `/board`; titulares por feed y artículo limpiado a texto por el servidor (`server/src/rss.ts`, sin LLM: párrafos del HTML), leído en el visor paginado. Titulares y artículos leídos quedan en la SD (`/.crosspoint/rss/`) para abrirlos sin WiFi; Atrás largo actualiza | ✅ | 1.5.27 |
| 3.5 | Álbum de fotos (mosaico Fotos, `PhotosActivity`): se suben desde `/board` y el navegador del teléfono hace todo el trabajo (escala a **480x800**, grises, difuminado Floyd-Steinberg y BMP de 2 bpp de 96 KB), el servidor solo guarda. El aparato lista, baja a `/Photos` de la SD y muestra a pantalla completa con refresco limpio; también lista los BMP que copies a mano. UP/DOWN pasan fotos, Atrás largo actualiza | ✅ | 1.5.33: hasta 1.5.32 no se veían por tres motivos encadenados — el aparato nunca pedía la lista al servidor al entrar, el BMP se generaba apaisado (una foto de teléfono quedaba en una franja de 480x288) y se dibujaba en una sola pasada en blanco y negro, que pinta de negro todo lo que no sea blanco puro. Ahora pide la lista siempre, el BMP viene del tamaño de la pantalla y se dibuja con el pipeline de 4 grises. Falta: salvapantallas de sueño con una foto |
| 3.6 | Clima detallado (`WeatherActivity`, **mosaico Clima**, en el lugar que tenía Juegos): ahora (temperatura, sensación, humedad, viento, salida y puesta del sol), próximas horas de a dos y seis días con barra de rango entre mínima y máxima y probabilidad de lluvia. `GET /api/hub/forecast` (Open-Meteo, caché de 15 min en el servidor y del último pronóstico en la SD, así abre sin WiFi); Atrás largo actualiza | ✅ | 1.5.32. La pantalla era **inalcanzable** hasta 1.5.31: se abría con OK mantenido y en esta placa OK no tiene pulsación larga (mantener OK apaga). Ahora tiene mosaico propio, la caché guarda `savedAt` y se refresca sola a la hora, y un 503 del servidor se muestra como "cargá el lugar en la web". Falta la temperatura interior del SHTC3 (0.6) |
