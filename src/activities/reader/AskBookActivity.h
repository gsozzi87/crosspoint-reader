...
// picks a preset question or types one; the text plus the question go to the
// device's own server (POST /api/ask, answered by Claude there) and the answer
// is shown in a paged viewer. Runs in place of the reader, like KOReader sync:
// WiFi + TLS need the heap the open book would otherwise hold, and the reader
// is re-entered through silentRestartToReader() on exit.
class AskBookActivity final : public Activity {
 public:
  ...
  bool preventAutoSleep() override { return state == CONNECTING || state == ASKING; }

 private:
  enum State { PICK, TYPING, CONNECTING, ASKING, ANSWER, FAILED };
  State state = PICK;
