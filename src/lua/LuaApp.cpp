...
int clampStroke(const lua_Integer v) {
  return static_cast<int>(std::max<lua_Integer>(1, std::min<lua_Integer>(v, 8)));
}
...
int cpLine(lua_State* L) {
  ...
  g_renderer->drawLine(clampCoord(luaL_checkinteger(L, 1), screenW),
                       clampCoord(luaL_checkinteger(L, 2), screenH),
                       clampCoord(luaL_checkinteger(L, 3), screenW),
                       clampCoord(luaL_checkinteger(L, 4), screenH), clampStroke(luaL_optinteger(L, 5, 1)), true);
  return 0;
}
