...
def generate_keys_header(
    ...
    # Este fork compila solo los idiomas del producto, asi que varios de esos
    # codigos ya no existen en el enum. La tabla igual tiene que conservar sus
    # POSICIONES (es un indice guardado en disco): los que no se compilan caen a
    # ingles, que es el respaldo de siempre.
    built = set(languages)
    dropped = [c for c in v1_codes if c not in built]
    lines.append("// V1 language.bin migration table (frozen enum order from 2f969a9)")
    if dropped:
        lines.append("// Idiomas de upstream no compilados en este fork -> caen a EN: " + ", ".join(dropped))
