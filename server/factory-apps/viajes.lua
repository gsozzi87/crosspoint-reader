...
local parent = {}
local function reload() data = cp.travel() or { trips = {} }; trip = data.trip end
local function rows()
...
local function draw_list(header, sub, list, mapper)
  ...
  local visible = math.max(1, (cp.height() - y - 45) // 66)
  local first = math.max(1, math.min(selected - visible + 1, math.max(1, #list - visible + 1)))
  for i = first, math.min(#list, first + visible - 1) do
    local a, b = mapper(list[i], i); row(y, a, b, i == selected); y = y + 66
  end
end
function on_open()
  reload()
  local resume = cp.load() or ""; cp.save(""); screen, selected = "trips", 1
  if trip and resume == "home:" .. trip.id then screen = "home" end
  if trip and resume == "guide:" .. trip.id and data.suggest and data.suggest.tripId == trip.id then screen = "guide" end
end
function on_key(k)
  ...
  if screen == "days" then parent = (trip.days or {})[selected] or { items = {} }; screen, selected = "items", 1; clamp(); return true end
...
function on_draw()
  ...
  if screen == "trips" then
    ...
    if #list == 1 then
      ...
      cp.text(32, 145, "No hay viajes sincronizados", 12, true); cp.text(32, 185, "Pulsa OK para actualizar", 10)
      cp.selection(24, 220, cp.width() - 48, 56); cp.text(38, 237, "Actualizar viajes", 12, true)
    else draw_list("Viajes", "Guías y planes disponibles sin conexión", list,
      function(v) return v.name or v.title or "Viaje", v.place or v.when or v.start or "" end) end
  elseif screen == "home" then
    local options = {{"Agenda del viaje", "Días, horarios y lugares"}, {"Guía inteligente", "Qué visitar y qué reservar"},
  ...
  elseif screen == "days" then draw_list(trip.name or "Viaje", "Agenda por día", trip.days or {}, function(v) return v.label ~= "" and v.label or v.date, v.date end)
  elseif screen == "items" then draw_list(parent.label or parent.date or "Día", parent.date or "", parent.items or {}, function(v) return ((v.at or "") ~= "" and (v.at .. "  ") or "") .. (v.title or "Actividad"), v.place or v.note or "" end)
  ...
  elseif screen == "guide" then draw_list("Guía inteligente", trip.name, data.suggest.lines or {}, function(v) return v, "" end); cp.text(32, cp.height() - 34, "OK: actualizar guía", 10) end
end
